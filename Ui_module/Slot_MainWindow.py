...
class Slot_MainWindow(object):
    ...
    def getTable1Item(self):
        n_row = self.tableWidget.rowCount()
        items_object = [self.tableWidget.item(i,0) for i in range(n_row)]
        self.itemsValue = [item.text() if item != None else item for item in items_object]
